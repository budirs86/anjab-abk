...
@section('container')
    ...
    <hr>
    <div class="">
        <form action="{{ route('admin.tugas-tambahan.store') }}" method="POST">
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="unsur" aria-label="Default select example" name="unsur_id">
                    ...
                    @foreach ($unsurs as $unsur)
                        <option value="{{ $unsur->id }}" @selected($unsur->id == old('unsur'))>{{ $unsur->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" id="jenisJabatan" aria-label="Default select example" name="jenis_jabatan_id">
                    ...
                    @foreach ($jenisJabatans as $jenisJabatan)
                        <option value="{{ $jenisJabatan->id }}" @selected($jenisJabatan->id == old('jenisJabatan'))>{{ $jenisJabatan->nama }}
                        </option>
                    @endforeach
                </select>
            </div>
